...
public class endMethods extends JPanel implements ActionListener{
	...
	public endMethods () {
		codeBgSubmit = Toolkit.getDefaultToolkit().createImage("codeBgSubmit.jpg");
		codeBgNext = Toolkit.getDefaultToolkit().createImage("codeBgNext.jpg");
		...
		if (lvlTwo.equalsIgnoreCase(userInput)) {
			...
			if (userScore == 5)
				if (userInput.indexOf("if") < removeSpaces(userInput).indexOf("(students>30)") && 
						removeSpaces(userInput).indexOf("(students>30)") < indexPrint && indexPrint < 
						userInput.toLowerCase().indexOf("('no')") && userInput.toLowerCase().indexOf("('no')") < userInput.indexOf(";"))
					userScore++;
	...
	public static int levelThree(String userInput) {
		...
		if (lvlThree.equalsIgnoreCase(userInput)) {
			...
			if (userInput.indexOf(";") != -1)
				userScore++;
			if (removeSpaces(userInput.toLowerCase()).indexOf("while(num>0)") != -1)
				userScore++;
			if (userInput.indexOf("{") != -1)
				userScore++;
			if (removeSpaces(userInput.toLowerCase()).indexOf("sum+=num;") != -1)
				userScore++;
			if (userInput.toLowerCase().indexOf("num--;") != -1)
				userScore++;
			if (userInput.indexOf("}") != -1)
				userScore++;
			if (userScore == 8)
				if(userInput.indexOf("int") < userInput.indexOf("=") && userInput.indexOf("=") < userInput.indexOf(";") &&
						...
						removeSpaces(userInput.toLowerCase()).indexOf("while(num>0)") < userInput.indexOf("{") && userInput.indexOf("{")
						< removeSpaces(userInput.toLowerCase()).indexOf("sum+=num;") && removeSpaces(userInput.toLowerCase()).indexOf("sum+=num;")
						< userInput.toLowerCase().indexOf("num--;") && userInput.toLowerCase().indexOf("num--;") < userInput.indexOf("}"))
					userScore++;
